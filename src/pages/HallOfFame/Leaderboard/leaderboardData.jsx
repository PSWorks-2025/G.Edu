...
const getStudentRankingData = async (studentsID) => {
  const students_data = await fetchStudentsData();
  const studentRankingData = Promise.all(
    studentsID
      .map(async (studentID) => {
...
export const getClassRankingByID = async (classID) => {
  ...
  const classRanking = await getStudentRankingData(studentsID);
  //   console.log('classRanking: ', classRanking);
  return classRanking;
};

// export const getAllClassRanking = () => {};
