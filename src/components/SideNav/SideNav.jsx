      ...
      <div className='flex flex-row h-full sticky z-2'>
        <nav style={{ backgroundColor: "#FCFCFC" }} className={`w-[260px] h-full px-6`}>
          ...
          <div className='h-[70px] items-center flex'>
            <p style={{color: primaryColors.gray,fontSize:20}} className='ROBOTO_FONTS ml-10'>G.Edu</p>
          </div>
          ...
          {/* Nav Items */}
            <ul className='mt-1'>
              {navItems.map((item) => {
                ...
                return (
                  <li
                  style={{ backgroundColor: active ? primaryColors.black : primaryColors.white, position: 'relative' }}
                  key={item.id}
                  className={`py-3 px-4 rounded-md mt-1`}
                  >
                  {active && <div style={{backgroundColor:primaryColors.black}} className='absolute left-0 translate-x-[-330%] top-0 bottom-0 w-2 rounded-r-2xl'></div>}
                  <Link to={`${item.path}`} onClick={() => setActiveTab(item.id)} className='flex items-center'>
                    <ion-icon style={{ fontSize:20, color: active ? primaryColors.white : primaryColors.gray }} name={item.icon}></ion-icon>
                    <span style={{ color: active ? primaryColors.white : primaryColors.gray, marginLeft:10 }}>{item.label}</span>
                  </Link>
                  </li>
                );
              })}
